...
class AgentServiceDeploymentTriggerTests(unittest.TestCase):
    ...
    def test_backend_approval_commits_before_triggering_qa_worker(
        self,
        get_db_conn,
        put_db_conn,
        locked_card,
        ...
        def assert_committed(scope, *, deployment_environment=None):
            self.assertTrue(committed["value"])
            self.assertEqual(scope, RepositoryScope.BACKEND)
            self.assertEqual(deployment_environment, "qa")
        ...
        require_ready.assert_not_called()
    ...
    @patch("backend.services.agent_service.put_db_conn")
    @patch("backend.services.agent_service.get_db_conn")
    def test_backend_deployment_retry_commits_before_triggering_qa_worker(
        self,
        get_db_conn,
        put_db_conn,
        locked_card,
